...
**Charm / allies** — The Reaper's Soul Harvest passive (`GameState.selected_character == "reaper"`, checked via `GameState.has_charm_passive()`) lets `Player._do_dash_knockback()` call `enemy.charm()` on any non-boss enemy it dashes through. Each enemy type (`Enemy`, `Wraith`, `Spider`) implements `charm()` / `_end_charm()` / `_charmed_physics()`: it leaves the `"enemies"` group, joins `"allies"`, tints green, and for `CHARM_DURATION` (8 s) chases and melees the nearest `"enemies"` node (crediting kill score to the player) without ever damaging the player, then reverts to hostile. Because charmed units leave `"enemies"`, they are automatically ignored by player auto-fire, `BoneSentry`, `Projectile`, dash knockback, and the fire bomb. Bosses (`Enemy.is_boss`) are immune. Character selection lives in `MainMenu._show_character_select()`.
