...
const BASE_TINT      := Color(0.72, 0.85, 1.0, 0.65)
const CHARM_TINT     := Color(0.4, 1.0, 0.7, 0.85)
...
func _anim_hurt() -> Animation:
	...
	a.track_insert_key(tm, 0.22, Color(0.72, 0.85, 1.0, 0.65))
	return a
...
func _anim_death() -> Animation:
	...
	a.track_set_path(ta, "Sprite2D:modulate")
	a.value_track_set_update_mode(ta, Animation.UPDATE_CONTINUOUS)
	a.track_insert_key(ta, 0.00, Color(4.0, 4.0, 4.0, 1.0))
	a.track_insert_key(ta, 0.07, Color(0.72, 0.85, 1.0, 0.65))
	a.track_insert_key(ta, 0.55, Color(0.72, 0.85, 1.0, 0.0))
	return a
...
# Charm — The Reaper's Soul Harvest passive turns this wraith into a
# temporary ally that hunts the horde, then reverts to hostile.
func charm() -> void:
	if _dead or is_charmed:
		return
	is_charmed    = true
...
func _charmed_physics(delta: float) -> void:
	...
	if _charm_target and is_instance_valid(_charm_target):
		...
		move_and_slide()
		if global_position.distance_to(_charm_target.global_position) < 24.0 and _contact_timer <= 0.0:
			_contact_timer = contact_cooldown
