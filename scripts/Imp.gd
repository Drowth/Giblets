...
const ANIM_PATH := "res://assets/enemies/imp"
const ANIM_SCALE := Vector2(0.51, 0.51)  # 128px HD frames, ~70px tall content -> ~36 world px
...
# Resting modulate the on-hit flash fades back to: charm green, else the imp tint.
func _flash_base() -> Color:
	return Color(0.3, 1.0, 0.3) if _charmed else IMP_TINT
